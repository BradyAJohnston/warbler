...
def download_whls(
    ...
    for platform in platforms:
        run_python(
            f"-m pip download {' '.join(required_packages)} --dest ./{DIR}/wheels --only-binary=:all: --python-version={python_version} --platform={platform.pypi_suffix}"
        )


def update_toml_whls(platforms):
    # Define the path for wheel files
    wheels_dir = f"{DIR}/wheels"
...
def clean_files(suffix: str = ".blend1") -> None:
    pattern_to_remove = f"{DIR}/**/*{suffix}"
...
def build_extension(split: bool = True) -> None:
    ...
    if split:
        subprocess.run(
            ...
            f" --split-platforms --source-dir {DIR} --output-dir .".split(" ")
        )
    else:
        subprocess.run(
            f"{bpy.app.binary_path} --command extension build "
            f"--source-dir {DIR} --output-dir .".split(" ")
        )
