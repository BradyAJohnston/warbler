...
TOML_PATH = "warbler/blender_manifest.toml"
WHL_PATH = "./warbler/wheels"
...
def download_whls(
    ...
    for platform in platforms:
        run_python(
            f"-m pip download {' '.join(required_packages)} --dest ./warbler/wheels --only-binary=:all: --python-version={python_version} --platform={platform.pypi_suffix}"
        )


def update_toml_whls(platforms):
    # Define the path for wheel files
    wheels_dir = "warbler/wheels"
...
def clean_files(suffix: str = ".blend1") -> None:
    pattern_to_remove = f"warbler/**/*{suffix}"
...
def build_extension(split: bool = True, blender_path: str = None) -> None:
    ...
    try:
        ...
        if not blender_path:
            raise AttributeError("Blender binary path is empty")
    ...
    if not blender_path:
        ...
        print(
            "  1. Run: blender --command extension build --split-platforms --source-dir warbler --output-dir ."
        )
    ...
    if split:
        subprocess.run(
            ...
            " --split-platforms --source-dir warbler --output-dir .".split(" ")
        )
    else:
        subprocess.run(
            f"{blender_path} --command extension build "
            "--source-dir warbler --output-dir .".split(" ")
        )


def get_all_dependencies_from_lock(package_name: str = "warbler") -> set:
...
def download_wheels_from_lock(
    ...
    # Get all dependencies for warbler
    print("Resolving dependencies from uv.lock...")
    all_deps = get_all_dependencies_from_lock("warbler")
